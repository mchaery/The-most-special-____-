...
<body>
    ...
    session_start();
    $query = "SELECT title, content, date, hit, id FROM travel WHERE number = $number";
    $result = $connect -> query($query);
    $rows = mysqli_fetch_assoc($result);

    $hit = "UPDATE travel SET hit = hit + 1 WHERE number = $number";
    $connect -> query($hit);
    ...
    <div class="read_btn">
        <button class="read_btn1" style="cursor: pointer" onclick="location.href='./travel.html'">목록</button>&nbsp;&nbsp;
        <?php
        if (isset($_SESSION['userid']) and $_SESSION['userid'] == $rows['id']) { ?>
            <button class="read_btn1" style="cursor: pointer" onclick="location.href='./modify4.php?number=<?= $number ?>'">수정</button>&nbsp;&nbsp;
            <button class="read_btn1" style="cursor: pointer" a onclick="ask();">삭제</button>

            <script>
                function ask() {
                    if (confirm("게시글을 삭제하시겠습니까?")) {
                        window.location = "./delete4.php?number=<?= $number ?>"
                    }
                }
            </script>
        <?php } ?>
